const { pullChanges } = require("../../services/gitlawb/gitService");
const logger = require("../../utils/synthLogger");

const schema = {
  name: "gitlawb-pull",
  description: "Pull latest changes from GitLawb remote",
  input: {
...
async function execute(input, opts = {}) {
  ...
  logger.agent(`[pull] ${input.remote || "origin"} → ${input.repoName || input.repoDir}`);
