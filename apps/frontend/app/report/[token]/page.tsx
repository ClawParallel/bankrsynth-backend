...
export async function generateMetadata({ params }: { params: Promise<{ token: string }> }): Promise<Metadata> {
  ...
  if (!data) {
    return {
      title: 'Token Report — SynthVirtual',
      description: 'AI-generated token intelligence report on SynthVirtual',
    }
  }
  ...
  return {
    title: `$${data.symbol} Intelligence Report — SynthVirtual`,
    description: `${data.verdict} | $${priceStr} | ${changeStr} | AI analysis on synthterminal.app`,
    openGraph: {
      title: `$${data.symbol} · ${data.verdict} — SynthVirtual`,
      description: data.synthesis?.analyze?.slice(0, 160) ?? `${data.symbol} analysis on SynthVirtual`,
      images: [{ url: ogUrl, width: 1200, height: 630 }],
      type: 'website',
    },
    twitter: {
      card: 'summary_large_image',
      title: `$${data.symbol} Analysis — SynthVirtual`,
    ...
    <main style={{ minHeight: '100dvh', paddingTop: 'clamp(52px,8vw,56px)', background: '#000', paddingBottom: '80px' }}>
      <div style={{ maxWidth: '900px', margin: '0 auto', padding: '0 16px' }}>
        ...
        <div style={{ padding: '10px 0', borderBottom: '1px solid rgba(0,255,65,0.1)', marginBottom: '16px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '8px' }}>
          <div>
            <div style={{ fontSize: '9px', color: 'rgba(0,255,65,0.35)', letterSpacing: '0.3em' }}>SYNTHVIRTUAL INTELLIGENCE REPORT</div>
        ...
        <div style={{ padding: '12px 0', fontSize: '9px', color: 'rgba(0,255,65,0.2)', letterSpacing: '0.1em', textAlign: 'center', lineHeight: 1.8 }}>
          Generated by SynthVirtual · synthterminal.app · $SYNTH · Built on Base · Launched on Virtuals Protocol<br />
